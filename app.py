...
PDF-bibliotheek: pdfplumber (text-only, geen extract_tables → weinig geheugen)
"""
import csv
import io
import os
...
logger.info("PDF-bibliotheek: pdfplumber (text-only)")
...
# ---------------------------------------------------------------------------
# Regex-patronen voor text-based parser
# ---------------------------------------------------------------------------

# Productlijn: CN8-code (8 cijfers), 4-cijfer code, M.U., qty, prijs, korting, waarde
PRODUCT_RE = re.compile(
    r'(\d{8})\s+\d{4}\s+\w+\s+(\d+)\s+[\d,.]+\s+[\d,.]+\s+([\d,.]+)'
)
# Gewicht in gram op de volgende lijn
WEIGHT_RE = re.compile(r'[Ww]eight\s+gr\s+(\d+)')
# Made In land-code
MADE_IN_RE = re.compile(r'Made\s+In\s+([A-Z]{2})', re.IGNORECASE)
# Overslaan: totaalrijen en ongewenste regels
SKIP_RE = re.compile(
    r'(TOTAL|TRANSPORT|IBAN|BANK|PAYMENT|DELIVERY|SIGNATURE|PALLET|CORRIERE|TAXABLE|EXEMPT)',
    re.IGNORECASE
)
# Factuurtotaal patronen
TOTAL_PATTERNS = [
...
# ---------------------------------------------------------------------------
# PDF parser — text-based (alleen extract_text, geen extract_tables)
# ---------------------------------------------------------------------------
...
def parse_pdf(pdf_bytes):
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        for pn, page in enumerate(pdf.pages, 1):
            ...
            # Text-based productlijn parsing
            text_lines = page_text.splitlines()
            pending = None  # wacht op gewichtslijn

            for line in text_lines:
                stripped = line.strip()

                # Gewichtslijn voor het pending product
                if pending is not None:
                    wm = WEIGHT_RE.search(stripped)
                    if wm:
                        pending["weight_kg"] = round(int(wm.group(1)) / 1000, 4)
                        mm = MADE_IN_RE.search(stripped)
                        if mm:
                            pending["made_in"] = mm.group(1).upper()
                    # Flush pending zodra we een nieuwe productlijn of gewicht gehad hebben
                    # Flush altijd na gewichtslijn; als volgende lijn ook product is, wordt pending overschreven
                    if wm or PRODUCT_RE.search(stripped):
                        lines.append(pending)
                        pending = None
                        if wm:
                            continue  # gewichtslijn verwerkt, ga verder

                # Skip ongewenste regels
                if SKIP_RE.search(stripped):
                    continue

                # Probeer productlijn te matchen
                pm = PRODUCT_RE.search(stripped)
                if pm:
                    cn8   = pm.group(1)
                    qty   = int(pm.group(2))
                    value = clean_num(pm.group(3))
                    if value is None or value <= 0:
                        continue
                    pending = {
                        "pn": pn, "cn8": cn8, "qty": qty,
                        "value": value, "weight_kg": 0.0, "made_in": "",
                    }

            # Einde pagina: flush pending product
            if pending is not None:
                lines.append(pending)
                pending = None
